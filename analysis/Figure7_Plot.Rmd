...
baseline = mean(res_df$rrmse[res_df$fit == "Mr.ASH"])
fig_main = ggplot(res_df) +
  stat_summary(aes(x = fit, y = rrmse), fun.y = mean, colour = col, geom = "point", shape = 18, size = 3) + 
  stat_summary(aes(x = fit, y = rrmse), colour = col, fun.y = mean, geom = "point") + 
  geom_boxplot(alpha = 0.1, aes(x = fit, y = rrmse, color = fit, fill = fit, lower = quantile(rrmse, .25), 
                                middle = mean(rrmse), upper = quantile(rrmse, .75)), outlier.alpha = 0, fatten = 0.9) +
