...
title     = ggdraw() + draw_label("Prediction Error (log-scale)", fontface = 'bold', size = 20) 
subtitle  = ggdraw() + draw_label("Scenario: IndepGauss + SparseNormal, n = 500, p = 2000, s = 20, pve = 0 - 0.9", fontface  = 'bold', size = 18) 
p0        = ggplot() + geom_blank() + theme_cowplot() + theme(axis.line = element_blank())
fig_main  = plot_grid(p0,p1,p0, nrow = 1, rel_widths = c(0.3,0.8,0.3))
fig       = plot_grid(title,subtitle,fig_main, ncol = 1, rel_heights = c(0.03,0.04,0.95))
fig
...
```{r fig71, fig.height=11, fig.width=15}
res_df       = readRDS("../results/highdimdiffp2.RDS")
method_list  = c("Mr.ASH","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn")
col          = gg_color_hue(13)[1:11]
shape        = c(19,17,24,25,9,3,11,4,5,7,8)
...
```{r fig8, fig.height=7, fig.width=15, fig.align = "center"}
...
```{r fig9, fig.height=13, fig.width=15}
...
ggsave("figure4_paper.pdf", fig, width = 18, height = 8)
```

```{r fig10, fig.height=8, fig.width=15}
...
fig_main = p1
title     = ggdraw() + draw_label("Prediction Error (log-scale)", fontface = 'bold', size = 20) 
subtitle  = ggdraw() + draw_label("Scenario: IndepGauss + PointNormal, n = 500, p = 10-2000, s = 10, pve = 0.5", fontface  = 'bold', size = 14) 
#fig1      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
fig1      = p1
```

```{r fig11, fig.height=8, fig.width=15}
res_df       = readRDS("../results/newhighdim.RDS")
method_list  = c("Mr.ASH","E-NET","Lasso","Ridge","SCAD","MCP","SCAD2","MCP2","L0Learn")
df           = data.frame()
p_range      = c(10,20,50,100,200,500,1000,2000,5000,10000)
...
#fig2      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
fig2      = p1
fig       = plot_grid(title, subtitle, plot_grid(fig1, fig2, nrow = 1, rel_widths = c(0.5,0.59)),
                      ncol = 1, rel_heights = c(0.05,0.05,0.95))
ggsave("figure5_paper.pdf", fig, width = 18, height = 8)
```

```{r fig100, fig.height=7, fig.width=15, fig.align = "center"}
...
p1 = ggplot(df) + geom_line(aes(x = s, y = pred, color = fit), size = 1) +
  ...
  geom_text(aes(x = 500, y = df$pred[8], label="E-NET ", col = "E-NET"), vjust = -0.2, hjust = -0.2) +
  geom_text(aes(x = 500, y = df$pred[16], label="Ridge  ", col = "Ridge"), vjust = 1, hjust = -0.22) +
  geom_text(aes(x = 500, y = df$pred[12], label="Lasso  ", col = "Lasso"), vjust = 0.2, hjust = -0.22) +
  geom_text(aes(x = 500, y = df$pred[20], label="SCAD  ", col = "SCAD"), vjust = 0.2, hjust = -0.22) +
  geom_text(aes(x = 500, y = df$pred[24], label="MCP   ", col = "MCP"), vjust = 0.2, hjust = -0.22) +
  geom_text(aes(x = 500, y = df$pred[28], label="L0Learn", col = "L0Learn"), vjust = 0.2, hjust = -0.22)
fig_main = p1
title     = ggdraw() + draw_label("Prediction Error (log-scale)", fontface = 'bold', size = 20) 
...
ggsave("PointNormal_Sparsity2.pdf", fig, width = 14, height = 8)
fig_main  = fig_main + 
...
```{r fig101, fig.height=7, fig.width=15, fig.align = "center"}
res_df = readRDS("../results/newridge10.RDS")
out = matrix(0,9,15)
lower = matrix(0,9,15)
upper = matrix(0,9,15)
...
df$fit = factor(df$fit, levels =  c("Mr.ASH","E-NET","Lasso","Ridge",
                                    ...
                                    "VarBVS","BayesB","Blasso","SuSiE","Mr.ASH.opt"))
df = df[df$s %in% c(1,5,50,500), ]
df = df[df$fit %in% c("Mr.ASH","Lasso","Ridge","L0Learn"),]
p1 = ggplot(df) + geom_line(aes(x = s, y = pred, color = fit), size = 1) +
  geom_point(aes(x = s, y = pred, color = fit, shape = fit), size = 3) +
  theme_cowplot(font_size = 14) +
  scale_x_continuous(trans = "log10", breaks = c(1,5,50,500)) +
  ...
  geom_text(aes(x = 500, y = df$pred[16], label="L0Learn", col = "L0Learn"), vjust = 0.2, hjust = -0.22)
fig_main = p1
title     = ggdraw() + draw_label("Prediction Error (log-scale)", fontface = 'bold', size = 20) 
subtitle  = ggdraw() + draw_label("Scenario: IndepGauss + PointNormal, n = 1000, p = 500, s = 1-500, pve = 0.5", fontface  = 'bold', size = 18) 
fig       = plot_grid(title,subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.05,0.95))
ggsave("PointNormal_Sparsity0.pdf", fig, width = 14, height = 8)
fig_main  = fig_main + 
    geom_ribbon(aes(x = s, ymin = lower, ymax = upper, color = fit, fill = fit),
                linetype = 2, size = 0, alpha = 0.1, colour = NA)
fig       = plot_grid(title,subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.05,0.95))
