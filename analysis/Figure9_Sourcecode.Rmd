...
```{r}
res_df       = readRDS("../backup/recentupdates/mcmc3.RDS")
method_list  = c("Mr.ASH.g","MCMC","Mr.ASH.g","MCMC","Mr.ASH.g","MCMC")
rho_list     = c(0,0,0.3,0.3,0.95,0.95)
df           = data.frame()
p_range      = c(200,1000,5000,20000)
for (i in 1:4) {
res_df[[i]]$fit   = rep(method_list, each = 20)
df                = rbind(df, data.frame(pred = c(colMeans(matrix(res_df[[i]]$pred, 20, 6))),
                                         time2 = apply(matrix(res_df[[i]]$time, 20, 6), 2, median),
                                         time = c(colMeans(matrix(res_df[[i]]$time, 20, 6))),
                                         p    = p_range[i],
                                         rho  = factor(rho_list),
                                         fit  = method_list))
}
