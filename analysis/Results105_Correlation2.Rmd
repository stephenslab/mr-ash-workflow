...
res_df       = readRDS("../results/equicorrnew50020020.RDS")
...
res_df       = readRDS("../results/equicorr50020020.RDS")
for (i in 1:6){
  res_df[[i]][1:20,] = tdat2[[i]][1:20,]
  res_df[[i]][221:240,] = tdat2[[i]][41:60,]
  res_df[[i]][241:260,] = tdat2[[i]][61:80,]
}
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
...
res_df       = readRDS("../results/ecorr5001000020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
                 "SCAD","MCP","L0Learn",
...
s            = 20
rho_list     = seq(0,0.9,0.1)
method_list  = c("lasso","scad2")
method_list2 = c("mr.ash", method_list, "mr.ash.order", "mr.ash.init", "mr.ash.order2", "mr.ash.init2")
...
for (i in 1:20) {
  data          = simulate_data(n, p, s = s, seed = i, signal = "normal", rho = rho,
                                design = "equicorrgauss", snr = 1 / (1 - rho))
  ...
  for (j in 1:length(method_list)) {
    ...
    if (method_list[j] == "lasso") {
      ...
      lasso.abs.order  = mr.ash.alpha:::abs.order(coef(fit$fit)[-1])
      lasso.beta       = as.vector(coef(fit$fit))[-1]
      lasso.time       = c(fit$t, fit$t2)
    }
    
    if (method_list[j] == "scad2") {
      scad.path.order = mr.ash.alpha:::path.order(fit$fit$fit)
      scad.abs.order  = mr.ash.alpha:::abs.order(c(coef(fit$fit)[-1]))
      scad.beta       = as.vector(coef(fit$fit))[-1]
      scad.time       = c(fit$t, fit$t2)
    }
  }
  fit         = fit.mr.ash2(data$X, data$y, data$X.test, data$y.test, seed = i,
                            update.order = lasso.abs.order,
                           sa2 = (2^((0:19) / 5) - 1)^2)
  pred[i,j+2] = fit$rsse / data$sigma / sqrt(n)
  time[i,j+2] = fit$t + lasso.time[2]
  
  fit         = fit.mr.ash2(data$X, data$y, data$X.test, data$y.test, seed = i,
                            update.order = scad.abs.order,
                           sa2 = (2^((0:19) / 5) - 1)^2)
  pred[i,j+4] = fit$rsse / data$sigma / sqrt(n)
  time[i,j+4] = fit$t + scad.time[2]
  print(c(pred[i,]))
}
