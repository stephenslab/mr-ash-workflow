...
sdat = readRDS("results/updateordernew1.RDS")
...
sdat$vobj = c(matrix(sdat$vobj, 75, 7)[,1] - matrix(sdat$vobj, 75, 7))
sdat$fit = factor(sdat$order, levels = method_list)
col = rev(c("darkblue","skyblue","dodgerblue","limegreen","gold","orange","salmon"))
...
sdat = readRDS("results/updateordernew2.RDS")
method_list  = c("random.fixed","scad.path.order","lasso.path.order","univ.abs.order",
                                 "lasso.abs.order","random.permute","oracle.abs.order")
sdat$vobj = c(matrix(sdat$vobj, 75, 7)[,1] - matrix(sdat$vobj, 75, 7))
sdat$fit = factor(sdat$order, levels = method_list)
...
for (i in 1:100) {
  data          = simulate_data(n, p, s = s, seed = i, signal = "normal", rho = 0.99,
...
tdat1 = data.frame(pred = c(pred[1:75,]), vobj = c(vobj[1:75,]), time = c(time[1:75,]),
                   numiter = c(numiter[1:75,]),
                   order = rep(method_list, each = 75))
```
