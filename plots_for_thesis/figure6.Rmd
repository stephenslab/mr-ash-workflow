...
for (iter in scenario_num) {
  ...
  for (i in 1:length(res_df)) {
    res_df[[i]]$pred[1:20] = newlist[[iter]][[i]]$pred[21:40]
...
for (iter in scenario_num) {
  ...
  res_df     = readRDS(datafile)
  for (i in 1:length(res_df)) {
    res_df[[i]]$pred[241:260] = newlist[[iter]][[i]]$pred[21:40]
...
for (iter in scenario_num) {
  ...
  res_df     = readRDS(datafile)
  for (i in 1:length(res_df)) {
    res_df[[i]]$pred[261:280] = newlist[[iter]][[i]]$pred[21:40]
    temp       = res_df[[i]][c(261:280,21:220),c("pred","time","fit")]
    temp$rrmse = temp$pred / temp$pred[1:20]
    temp$rpred = temp$pred / apply(matrix(temp$pred, 20, 11), 1, min)
    df         = rbind(df, temp)
  }
