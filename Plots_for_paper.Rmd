...
```{r fig10, fig.height=11, fig.width=15}
res_df       = readRDS("results/highdimdiffnpnew.RDS")
method_list  = c("Mr.ASH","E-NET","Lasso","Ridge","SCAD","MCP","SCAD2","MCP2","L0Learn")
df           = data.frame()
p_range      = c(20,50,100,200,500,1000,2000,5000,10000,20000)
...
fig10      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```
...
df$size = 0.5
df$size[1:9] = 1.2
df = df[df$fit %in% c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","Mr.ASH.opt"),]
p1 = ggplot(df) + geom_line(aes(x = s, y = pred, color = fit), size = df$size) +
  geom_point(aes(x = s, y = pred, color = fit, shape = fit), size = 3) +
  theme_cowplot(font_size = 14) +
  scale_x_continuous(trans = "log10", breaks = c(1,2,5,10,20,50,100,200,500)) +
...
res_df       = readRDS("results/BayesLinReg2.RDS")
method_list  = c("Mr.ASH","VarBVS","VarBVS2","BayesB","Blasso","SuSiE","Mr.ASH.opt")
method_level = c("Mr.ASH","E-NET","Lasso","Ridge",
                 "SCAD","MCP","L0Learn",
                 "VarBVS","BayesB","Blasso","SuSiE","VarBVS2","Mr.ASH.opt")
...
res_df       = readRDS("results/BayesLinReg1.RDS")
method_list  = c("Mr.ASH","VarBVS","VarBVS2","BayesB","Blasso","SuSiE","Mr.ASH.opt")
method_level = c("Mr.ASH","E-NET","Lasso","Ridge",
                 "SCAD","MCP","L0Learn",
                 "VarBVS","BayesB","Blasso","SuSiE","VarBVS2","Mr.ASH.opt")
col          = gg_color_hue(13)[c(1,8:13)]
shape        = c(19,17,24,25,9,3,11,4,5,7,8,2,1)[c(1,8:13)]
s_range      = c(20,50,200,500,2000,10000,20000)
sdat         = data.frame()
for (i in 1:7) {
  sdat = rbind(sdat, data.frame(pred = colMeans(matrix(res_df[[i]]$pred, 20, 7)),
                                time = colMeans(matrix(res_df[[i]]$time, 20, 7)),
...
subtitle  = ggdraw() + draw_label("Scenario: IndepGauss + PointNormal, n = 500, p = 20000, s = 1-20000, pve = 0-0.9", fontface  = 'bold', size = 14) 
...
col          = gg_color_hue(13)[c(1,8:13)]
shape        = c(19,17,24,25,9,3,11,4,5,7,8,2,1)[c(1,8:13)]
p_range      = c(20,50,200,500,2000,10000,20000,100000)
sdat         = data.frame()
for (i in 1:7) {
  sdat = rbind(sdat, data.frame(pred = colMeans(matrix(res_df[[i]]$pred, 20, 7)),
                                time = colMeans(matrix(res_df[[i]]$time, 20, 7)),
...
sdat     = rbind(sdat, data.frame(pred = 0,
                                  time = c(92.45325, 206.78125, 0, 184.80950, 185.48075,  83.94225, 0),
                                  fit   = method_list, p = 100000))
sdat$fit = factor(sdat$fit, levels = method_level)
sdat     = sdat[sdat$fit %in% c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE"),]
sdat$size = 0.5
sdat$size[1:8] = 1.2
