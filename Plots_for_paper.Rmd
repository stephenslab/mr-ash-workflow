...
df$lt = "solid"
df$lt[31:36] = "dashed"
fig_dummy0 = ggplot(df) + geom_line(aes(x = x, y = pred, color = fit), size = df$size, linetype = df$lt) +
  geom_point(aes(x = x, y = pred, color = fit, shape = fit), size = 2.5) +
  theme_cowplot(font_size = 14) +
  scale_x_continuous(breaks = c(1,2,3,4,5,6),
                     labels = c("0","0.3","0.6","0.9","0.95","0.99")) +
  ...
  scale_y_continuous(trans = "log10", breaks = c(1,1.05,1.1,1.15,1.2,1.3,1.4)) +
  coord_cartesian(ylim = c(1,1.3))
p1        = fig_dummy0 + theme(legend.position = "none")
fig_main  = p1
suptitle  = ggdraw() + draw_label("Varying Correlation", fontface  = 'bold', size = 14) 
subtitle  = ggdraw() + draw_label("Scenario: EquiCorrGauss + PointNormal, n = 500, p = 200, s = 20, pve = 0.5", fontface  = 'bold', size = 14) 
...
title    = ggdraw() + draw_label("Prediction Error (RMSE / sigma)", fontface = 'bold', size = 20)
legend <- get_legend(fig_dummy0 + theme(legend.box.margin = margin(0, 0, 0, 12)))
...
```{r fig41, fig.height=11, fig.width=15}
df         = readRDS("results/design4.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order", "Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order", "Mr.ASH.init",
                 "E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "VarBVS","BayesB","Blasso","SuSiE")
design       = c("Real Genotype","Locally Decaying Correlation, rho = 0.9", "Changepoint + OneBigBump","Changepoint + OneSmallChange")
limits       = list(c(1,1.35),c(1,1.3),c(0,0.6),c(0,2.0))
fig41        = list()
col          = gg_color_hue(13)[c(1,12,13,2:11)]
for (i in 1:4) {
  df[[i]]$fit = rep(method_list, each = 20)
  df[[i]]$fit = factor(df[[i]]$fit, levels = method_level)
  fig41[[i]]    = ggplot(df[[i]]) +
  stat_summary(aes(x = fit, y = pred), fun.y = mean, colour = col, geom = "point", shape = 18, size = 3) + 
  stat_summary(aes(x = fit, y = pred, ymin = ..y.., ymax = ..y..), colour = col, fun.y = mean, geom = "errorbar",
               width = 0.75, size = 0, linetype = "dotted") + 
  ...
  labs(x           = "") + coord_cartesian(ylim = limits[[i]]) + 
  theme_cowplot(font_size = 14) + 
  theme(axis.line    = element_blank(),
        axis.text.x  = element_text(angle = 45,hjust = 1),
        legend.position = "none")
  subtitle[[i]] = ggdraw() + draw_label(design[i], fontface = 'bold', size = 14)
  fig41[[i]]    = plot_grid(subtitle[[i]], fig41[[i]], ncol = 1, rel_heights = c(0.05,0.95))
}
```

```{r figure9, fig.height=14, fig.width=16}
figure9  = plot_grid(fig41[[1]], fig41[[2]], fig41[[3]], fig41[[4]], nrow = 2)
title    = ggdraw() + draw_label("Different Design", fontface = 'bold', size = 20) 
figure9  = plot_grid(title, figure9, ncol = 1, rel_heights = c(0.04,0.95))
ggsave("figures/figure9_paper.pdf", figure9, width = 18, height = 15)
```

```{r fig42, fig.height=11, fig.width=15}
res_df       = readRDS("results/localcorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
                 "SCAD","MCP","L0Learn",
...
col   = gg_color_hue(13)[c(1:3,5:7,12,13)]
shape = c(19,17,24,25,9,3,11,4,5,7,8,10,1)[c(1:3,5:7,12,13)]
df = data.frame(x = rep(1:length(rho_list),length(ind)), rho = rep(rho_list, length(ind)),
                pred = c(out), fit = rep(colnames(out), each = length(rho_list)))
df$fit = factor(df$fit, levels =  c("Mr.ASH","E-NET","Lasso","Ridge",
                                    "SCAD","MCP","L0Learn","E-NET2","Lasso2","Ridge2",
...
fig_dummy = ggplot(df) + geom_line(aes(x = x, y = pred, color = fit), size = df$size, linetype = df$lt) +
  ...
  scale_x_continuous(breaks = c(1,2,3,4,5,6),
                     labels = c("0","0.3","0.6","0.9","0.95","0.99")) +
  labs(y = "false discovery proportion", x = "correlation across columns of X (rho)") +
  theme(axis.line = element_blank(),
        plot.title = element_text(hjust = 0.5)) +
  scale_color_manual(values = col) +
  scale_shape_manual(values = shape) +
...
p1        = fig_dummy + theme(legend.position = "none")
fig_main  = p1
subtitle  = ggdraw() + draw_label("False Discovery Proportion (# of false discoveries / # of discoveries)", fontface  = 'bold', size = 14) 
fig42      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r fig43, fig.height=11, fig.width=15}
res_df       = readRDS("results/localcorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
...
out = matrix(0,6,13)
for (i in 1:6) {
  out[i,] = colMeans(matrix(res_df[[a[i]]]$pow, 20, 13))
}
colnames(out) = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge",
                  "SCAD","MCP","L0Learn",
                  "Mr.ASH.order",
...
fig_dummy = ggplot(df) + geom_line(aes(x = x, y = pred, color = fit), size = df$size, linetype = df$lt) +
  ...
  scale_x_continuous(breaks = c(1,2,3,4,5,6),
                     labels = c("0","0.3","0.6","0.9","0.95","0.99")) +
  labs(y = "power", x = "correlation across columns of X (rho)") +
  theme(axis.line = element_blank(),
        plot.title = element_text(hjust = 0.5)) +
  scale_color_manual(values = col) +
  scale_shape_manual(values = shape) +
...
p1        = fig_dummy + theme(legend.position = "none")
fig_main  = p1
subtitle  = ggdraw() + draw_label("Power (# of true discoveries / # of nonzero coefficients)", fontface  = 'bold', size = 14) 
fig43      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r fig44, fig.height=11, fig.width=15}
res_df       = readRDS("results/localcorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
...
col   = gg_color_hue(13)[c(1,8,9,11,12,13)]
shape = c(19,17,24,25,9,3,11,4,5,7,8,10,1)[c(1,8,9,11,12,13)]
df = data.frame(x = rep(1:length(rho_list),length(ind)), rho = rep(rho_list, length(ind)),
                pred = c(out), fit = rep(colnames(out), each = length(rho_list)))
df$fit = factor(df$fit, levels =  c("Mr.ASH","E-NET","Lasso","Ridge",
                                    "SCAD","MCP","L0Learn","E-NET2","Lasso2","Ridge2",
...
fig_dummy = ggplot(df) + geom_line(aes(x = x, y = pred, color = fit), size = df$size, linetype = df$lt) +
  ...
  scale_x_continuous(breaks = c(1,2,3,4,5,6),
                     labels = c("0","0.3","0.6","0.9","0.95","0.99")) +
  labs(y = "false discovery proportion (fdp)", x = "correlation across columns of X (rho)") +
  theme(axis.line = element_blank(),
        plot.title = element_text(hjust = 0.5)) +
  scale_color_manual(values = col) +
  scale_shape_manual(values = shape) +
...
fig_main  = p1
subtitle  = ggdraw() + draw_label("False Discovery Proportion (# of false discoveries / # of discoveries)", fontface  = 'bold', size = 14) 
fig44      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r fig45, fig.height=11, fig.width=15}
res_df       = readRDS("results/localcorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
...
fig_main  = p1
subtitle  = ggdraw() + draw_label("Power (# of true discoveries / # of nonzero coefficients)", fontface  = 'bold', size = 14) 
fig45     = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r fig42.1, fig.height=11, fig.width=15}
res_df       = readRDS("results/equicorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
                 "SCAD","MCP","L0Learn",
...
fig_main  = p1
subtitle  = ggdraw() + draw_label("False Discovery Proportion (# of false discoveries / # of discoveries)", fontface  = 'bold', size = 14) 
fig42.1      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r fig43.1, fig.height=11, fig.width=15}
res_df       = readRDS("results/equicorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
...
fig_main  = p1
subtitle  = ggdraw() + draw_label("Power (# of true discoveries / # of nonzero coefficients)", fontface  = 'bold', size = 14) 
fig43.1      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r fig44.1, fig.height=11, fig.width=15}
res_df       = readRDS("results/equicorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
...
fig_main  = p1
subtitle  = ggdraw() + draw_label("False Discovery Proportion (# of false discoveries / # of discoveries)", fontface  = 'bold', size = 14) 
fig44.1      = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r fig45.1, fig.height=11, fig.width=15}
res_df       = readRDS("results/equicorrfdr500200020.RDS")
method_list  = c("Mr.ASH","VarBVS","BayesB","Blasso","SuSiE","E-NET","Lasso","Ridge","SCAD","MCP","L0Learn",
                 "Mr.ASH.order","Mr.ASH.init")
method_level = c("Mr.ASH","Mr.ASH.order","Mr.ASH.init","E-NET","Lasso","Ridge",
...
fig_main  = p1
subtitle  = ggdraw() + draw_label("Power (# of true discoveries / # of nonzero coefficients)", fontface  = 'bold', size = 14) 
fig45.1     = plot_grid(subtitle,fig_main, ncol = 1, rel_heights = c(0.05,0.95))
```

```{r figure10, fig.height=14, fig.width=16}
figure10.1 = plot_grid(fig42, fig43, nrow = 1)
legend    <- get_legend(fig_dummy1 + theme(legend.box.margin = margin(0, 0, 0, 12)))
figure10.1  = plot_grid(figure10.1, legend, nrow = 1, rel_widths = c(0.9,0.1))
figure10.2 = plot_grid(fig44, fig45, nrow = 1)
legend    <- get_legend(fig_dummy0 + theme(legend.box.margin = margin(0, 0, 0, 12)))
figure10.2 = plot_grid(figure10.2, legend, nrow = 1, rel_widths = c(0.9,0.1))
figure10   = plot_grid(figure10.1, figure10.2, ncol = 1)
title      = ggdraw() + draw_label("Variable Selection Performance", fontface = 'bold', size = 20) 
subtitle   = ggdraw() + draw_label("Scenario: EquiCorrGauss + PointNormal, n = 500, p = 2000, s = 20, pve = 0.5", fontface  = 'bold', size = 14) 
figure10  = plot_grid(title, subtitle, figure10, ncol = 1, rel_heights = c(0.04,0.04,0.95))
ggsave("figures/figure10_paper.pdf", figure10, width = 18, height = 15)
```
