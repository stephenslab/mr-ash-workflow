...
rate = matrix(0,20,3)
pred = matrix(0,20,3)
pve_list = c(0.0001,0.1,0.5,0.9,0.9999)
tdat = list()
...
for (i in 1:20) {
  ...
  t.mr.ash2           = system.time(
    fit.mr.ash2        <- mr.ash(X = data$X, y = data$y, sa2 = sa2, method = "em3",
...
for (i in 1:20) {
  ...
  print(pred[i,] <- c(norm(pred1, '2'), norm(pred2, '2'), norm(pred3, '2')) / sqrt(n) / data$sigma)
  a = fit.mr.ash1$sigma / data$sigma^2
  b = fit.mr.ash2$sigma / data$sigma^2
  c = fit.varbvsmix$sigma / data$sigma^2
  time[i,] = c(t.mr.ash1[3], t.mr.ash2[3], t.varbvsmix[3])
